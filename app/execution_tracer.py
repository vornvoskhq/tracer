...
# Standalone tracer script template used by trace_command.
# This avoids relying on the legacy create_main_execution_tracer implementation,
# which embeds a large template and has proven error-prone to modify.
...
def should_trace(filename, function_name):
    ...
    include_patterns = [
        "$CODEBASE_NAME_LOWER",  # vgmini
        "/src/",                 # src directory files
        "src/",                  # src directory files (relative)
        "$TARGET_DIR",           # anything in target directory
        "vgmini.py",            # main entry point
        "/vgmini/",             # vgmini directory
    ]
    ...
    # This catches files that might not match the patterns above
    try:
        import os
        rel_filename = os.path.relpath(filename)
        if rel_filename.endswith('.py') and not rel_filename.startswith('..') and not '/.venv/' in rel_filename:
            # It's a Python file in or below the current directory (not in venv)
            skip_functions = {
                "__enter__",
                "__exit__",
                "__del__", 
        ...
        def patched_execv(*args, **kwargs):
            ...
            if len(args) >= 2:
                ...
                if '.venv' in python_path and len(argv_list) > 1:
                    # This is a venv restart - we need to simulate it by continuing
                    # execution as if we were already in the venv
                    print("🔄 Detected venv restart - simulating successful restart")
                    
                    # Update sys.executable to match what would happen after restart
                    sys.executable = python_path
                    
                    # Continue execution instead of restarting
...
def build_tracer_script(
    ...
    """
    Build the tracer script for a given target codebase with proper path handling.

    This version includes fixes for:
    - Virtual environment restart handling
    - Parent directory path inclusion for dependencies like cluster2
    - Proper module resolution
    """
    target_dir_escaped = target_dir.replace("\\", "\\\\")
    command_args_repr = repr(command_args)
    
    # Enhanced template with path fixes
...
# Add parent directory to path for dependencies like cluster2
parent_dir = os.path.dirname(os.path.abspath("$TARGET_DIR"))
if parent_dir not in sys.path:
    sys.path.insert(0, parent_dir)
    print(f"➕ Added parent dir to path: {parent_dir}")

print("🚀 $CODEBASE_TITLE Execution Tracer (Enhanced)")
print(f"Python: {sys.executable}")
print("Entry Point: $ENTRY_POINT")
print(f"Args: $COMMAND_ARGS")
print("=" * 60)
